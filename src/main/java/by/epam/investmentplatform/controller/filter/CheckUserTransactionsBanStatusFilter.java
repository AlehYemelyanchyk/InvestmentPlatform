...
@WebFilter(filterName = "CheckUserTransactionsBanStatusFilter")
public class CheckUserTransactionsBanStatusFilter extends AbstractFilter {
    ...
    public void doFilter(HttpServletRequest req, HttpServletResponse resp, FilterChain filterChain) throws ServletException, IOException {
        List<Integer> bannedTransactionsUsersIdList;
        try {
            bannedTransactionsUsersIdList = userService.getBannedTransactionsUsersIdList();
        } catch (ServiceException e) {
            LOGGER.error("CheckUserTransactionsBanStatusFilter error: ", e);
        ...
        if (isBanned(bannedTransactionsUsersIdList, userId)) {
            String requestUrl = req.getRequestURI();
            req.getSession().setAttribute(NamesConstants.REDIRECT_LINK, requestUrl);
            req.setAttribute(NamesConstants.ERROR_ATTRIBUTE, "Sorry, but your transactions were banned due a suspicious activity. For further information, please, contact our support team.");
